...
public class StatsServiceImpl implements StatsService {
    ...
    public void recordVisitor(HttpServletRequest request, String userId) {
        ...
        if (justLoggedOut != null && justLoggedOut && userId == null) {
			// 로그아웃 후 /foodhub 리다이렉트로 인한 요청은 카운트 증가 안 함
        ...
        if (userId == null) {
        	...
        	if (hasVisited == null || !hasVisited) {
        		// 해당 사용자의 첫 방문: 로그 기록 및 카운트 증가
        		if (lastIpVisit == null && lastLogoutTime == null) {
					// 비로그인 방문 기록도 없고 로그아웃 기록도 없으면 카운트 증가
        			shouldIncrement = true;
				}
        		session.setAttribute(sessionUserKey, true);
        		session.setAttribute("preUserId", userId);
        	}
    	}
